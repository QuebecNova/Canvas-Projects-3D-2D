...
export class Draw3D implements Singleton {
    ...
    private stats() {
        setInterval(() => {
            const fps = round(
                1 / (this.deltaTime === 0 ? 1 / 60 : this.deltaTime)
            )
    ...
    private setMatrices() {
        ...
        const scaleMatrix = Mat4.Scaling.xyz(
            Draw3D.scale,
            Draw3D.scale,
            Draw3D.scale
        )
    ...
    private addEventListeners() {
        ...
        document.addEventListener('pointerlockchange', () => {
            ...
            document.onkeydown = (e) => {
                ...
                if (
                    ...
                    key &&
                    Object.hasOwn(this.keyPressed, key)
                ) {
        ...
        this.canvas.addEventListener('click', async () => {
            ...
            if (
                ...
                now - pointerLockActivatedAt < minDelay
            ) {
                return
            }
    ...
    private actions = {
        ...
        right: (deltaTime: number) => {
            this.camera.x +=
                sin(this.camera.β + pi / 2) *
                this.camera.speed *
                deltaTime *
                Draw3D.scale
            this.camera.z +=
                cos(this.camera.β + pi / 2) *
                this.camera.speed *
                deltaTime *
                Draw3D.scale
        },
